...
AZUL_PRIMARIO = "#12B7FF"
AZUL_OSCURO = "#0B4F8A"
AZUL_PANEL_BORDE = "#1E3A5F"
NEGRO_FONDO = "#070B14"
NEGRO_PANEL = "#111A2B"
NEGRO_INPUT = "#0C1626"
BLANCO = "#FFFFFF"
...
# Tamaños tipográficos consistentes en toda la app
FS_BASE = "15px"
FS_LABEL = "13.5px"
FS_SMALL = "12.5px"
...
<style>
    ...
    * {{ color: {BLANCO}; }}
    ...
    .stApp, [data-testid="stAppViewContainer"], [data-testid="stMain"] {{
        background: linear-gradient(180deg, {NEGRO_FONDO} 0%, #04060C 100%) !important;
    }}
    ...
    section[data-testid="stSidebar"] {{
        ...
        border-right: 1px solid {AZUL_PANEL_BORDE};
    }}
    section[data-testid="stSidebar"] .block-container {{
        padding-top: 1.2rem;
    }}

    /* Encabezados de sección en el sidebar (### 1. ..., ### 2. ...) */
    section[data-testid="stSidebar"] h2 {{
        font-size: 1.15rem !important;
        color: {BLANCO} !important;
        border-bottom: 1px solid {AZUL_PANEL_BORDE};
    ...
    div[data-baseweb="radio"] label {{
        ...
        font-size: {FS_LABEL} !important;
        font-weight: 600 !important;
        opacity: 1 !important;
    }}

    /* --------- CAMPOS DE ENTRADA (text/number/select) --------- */
    .stTextInput input,
    .stNumberInput input,
    div[data-baseweb="select"] > div,
    div[data-baseweb="base-input"] {{
        background-color: {NEGRO_INPUT} !important;
        color: {BLANCO} !important;
        border: 1px solid {AZUL_PANEL_BORDE} !important;
    ...
    div[data-baseweb="select"] > div:focus-within {{
        border-color: {AZUL_PRIMARIO} !important;
        box-shadow: 0 0 0 1px {AZUL_PRIMARIO} !important;
    }}
    ...
    svg {{ fill: {GRIS_CLARO} !important; }}

    /* Radio buttons (Plazo VaR / Periodicidad) */
    .stRadio [role="radiogroup"] label {{
        background-color: {NEGRO_INPUT};
        border: 1px solid {AZUL_PANEL_BORDE};
        border-radius: 6px;
        padding: 4px 10px;
        margin-right: 6px;
    }}
    ...
    .fintech-header {{
        background: linear-gradient(90deg, {NEGRO_PANEL} 0%, {AZUL_OSCURO} 100%);
    ...
    .stButton>button, .stDownloadButton>button {{
        ...
        color: {NEGRO_FONDO} !important;
        font-weight: 700;
        border: none;
        border-radius: 6px;
        font-size: {FS_BASE} !important;
    }}
    .stButton>button *, .stDownloadButton>button * {{ color: {NEGRO_FONDO} !important; }}
    ...
    /* --------- TARJETA CONTENEDORA PARA GRÁFICAS --------- */
    .chart-card {{
        background-color: {NEGRO_PANEL};
        border: 1px solid {AZUL_PANEL_BORDE};
        border-radius: 10px;
    ...
    /* --------- TABLA DE RESULTADOS (HTML propia) --------- */
    .tabla-wrap {{
        overflow-x: auto;
        border: 1px solid {AZUL_PANEL_BORDE};
        border-radius: 10px;
    ...
    table.tabla-fintech thead th:first-child {{ text-align: left; }}
    table.tabla-fintech tbody td {{
        padding: 9px 12px;
        text-align: right;
        white-space: nowrap;
        border-bottom: 1px solid {AZUL_PANEL_BORDE};
    ...
    table.tabla-fintech tbody td:first-child {{
        text-align: left;
        font-weight: 700;
        color: {AZUL_PRIMARIO} !important;
    }}
    table.tabla-fintech tbody tr:nth-child(odd) {{ background-color: {NEGRO_PANEL}; }}
    table.tabla-fintech tbody tr:nth-child(even) {{ background-color: {NEGRO_INPUT}; }}
    table.tabla-fintech tbody tr:hover {{ background-color: {AZUL_OSCURO}; }}
    .valor-pos {{ color: {VERDE} !important; }}
    .valor-neg {{ color: {ROJO} !important; }}
    .valor-amb {{ color: {AMBAR} !important; }}

    hr {{ border-color: {AZUL_PANEL_BORDE}; }}
...
def layout_grafica(**overrides):
    """Layout base consistente (fuente, colores, márgenes) para todas las
    gráficas de Plotly, evitando que los títulos de los ejes se corten."""
    base = dict(
        template="plotly_dark",
        paper_bgcolor=NEGRO_PANEL,
        plot_bgcolor=NEGRO_PANEL,
        font=dict(family="Arial, Helvetica, sans-serif", color=BLANCO, size=13),
        title_font=dict(family="Arial, Helvetica, sans-serif", color=BLANCO, size=15),
        legend=dict(font=dict(color=GRIS_CLARO, size=12)),
        margin=dict(l=70, r=30, t=55, b=50),
        xaxis=dict(automargin=True, gridcolor=AZUL_PANEL_BORDE, color=GRIS_CLARO,
                    title_font=dict(color=GRIS_CLARO)),
        yaxis=dict(automargin=True, gridcolor=AZUL_PANEL_BORDE, color=GRIS_CLARO,
                    title_font=dict(color=GRIS_CLARO)),
    )
...
if "df_resultados" in st.session_state:
    ...
    with tab_resumen:
        ...
        # Columnas cuyo signo se resalta (verde = positivo/favorable, rojo = negativo)
        COLUMNAS_SIGNO = {"Rentabilidad Anualizada", "Índice Sharpe", "Índice Treynor",
                           "Alpha (Jensen)", "Correlación Pearson"}

        def construir_tabla_html(df: pd.DataFrame, formatos: dict) -> str:
            """Genera una tabla HTML con estilo propio (independiente del tema
            de Streamlit) para garantizar contraste y tipografía consistentes."""
            encabezados = "".join(f"<th>{col}</th>" for col in df.columns)
            filas_html = []
            for ticker, fila in df.iterrows():
                celdas = [f"<td>{ticker}</td>"]
                for col in df.columns:
                    val = fila[col]
                    if pd.isna(val):
                        celdas.append("<td>—</td>")
                        continue
                    texto = formatos.get(col, "{}").format(val)
                    clase = ""
                    if col == "Beta":
                        clase = "valor-amb" if val > 1 else ""
                    elif col in COLUMNAS_SIGNO:
                        clase = "valor-pos" if val >= 0 else "valor-neg"
                    celdas.append(f'<td class="{clase}">{texto}</td>')
                filas_html.append(f"<tr>{''.join(celdas)}</tr>")

            return f"""
            <div class="tabla-wrap">
              <table class="tabla-fintech">
                <thead><tr><th>Ticker</th>{encabezados}</tr></thead>
                <tbody>{''.join(filas_html)}</tbody>
              </table>
            </div>
            """

        st.markdown(construir_tabla_html(df_resultados, fmt), unsafe_allow_html=True)
        ...
        with c1:
            ...
            fig.update_layout(**layout_grafica(yaxis_tickformat=".1%"))
        ...
        with c2:
            ...
            fig2.update_layout(**layout_grafica())
            st.markdown('<div class="chart-card">', unsafe_allow_html=True)
    ...
    with tab_detalle:
        ...
        fig_precio.update_layout(**layout_grafica(height=350, showlegend=False))
        st.markdown('<div class="chart-card">', unsafe_allow_html=True)
    ...
    with tab_graficas:
        ...
        if len(x) > 1:
            ...
            fig_reg.update_layout(**layout_grafica(
